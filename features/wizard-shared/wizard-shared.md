...
- **Billing (explicit):** **Image credit consumption** is **coupled to preview generation** — triggers, batching, idempotency, and in-UI estimates are defined in **`features/wizard-preview/wizard-preview.md`** and `PRD_Obra.md` §6. **`image_mode` and `image_style` (and palette-linked context)** are **inputs** to that pipeline alongside §6 editor behavior.
