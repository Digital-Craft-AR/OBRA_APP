...
Typical tables: users/profiles, projects (`content_locale`, **`content_source`**, optional **`author`**, **`topic`**, **`problem`**, **`target_avatar`**, **`structure_completed_at`**, lifecycle fields), **`project_structure_drafts`** (pre–design JSON; **optimistic locking** via `updated_at`), design system row(s), ebooks (main/bonus/bump; **join** `projects` for locale/author/persona/problem), **`project_content_progress`** (`current_phase` **CHECK** in DB: `upload_alignment` … `complete`; optimistic `updated_at`), **`project_manuscripts`** (upload branch: Storage path + metadata; one current row per project), chapters (**`approved_at`** per chapter in Contenido), **`content_chat_threads`** / **`content_chat_messages`** (scoped IA chat; **user** turns idempotent via `client_message_id` per thread; **assistant** messages persisted **after** stream completes — MVP; **deleted** on confirmed avatar/problem reset), images, credits ledger, subscriptions/payments as needed. **Package limits** (1 main, <=5 bonuses, <=2 bumps) are enforced in both **app/API** and **DB triggers**. **`landing_pages`** is **post-MVP**. Storage bucket for project assets (e.g. `project-images`). **RLS** on all user data. Schema detail: `ARQUITECTURA_Obra.md` §3 (duplicate checklist; **avatar/problem reset** operation order; **no** chat copy on duplicate; chat **wiped** on reset).
